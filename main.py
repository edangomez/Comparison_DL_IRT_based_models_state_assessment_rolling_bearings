...
def main():
    parser = argparse.ArgumentParser(
        description="Caltech 101 classification"
    )
    parser.add_argument(
        "--mode",
        type=str,
    ...
    parser.add_argument(
        "--experiment", 
        type=str, 
        default="fv-HynoRS-interpol"
    )
    parser.add_argument(
        "--dtype",
        type=str,
        default="hybrid2",
        choices=["img", "irt", "hybrid", "hybrid2", "hybrid3"],
    ...
    parser.add_argument(
        "--version", type=str, default=False, help="version of EffcientNet"
    )
    parser.add_argument(
        "--load",
        action="store_true",
    ...
    experiment = {
        "bl_img": {
            "w_route": "./Experiments_img/efficientNEt_noDA1_1/epoch_40.pt",
            "dtype": "img",
            "size": "b0",
            "resize_type": None,
            "version": "b0",
        ...
        "bl_irt": {
            "w_route": "./Experiments_irt/efficientNEt_noDA1/epoch_40.pt",
            "dtype": "irt",
            "size": "b0",
            "resize_type": None,
            "version": "b0",
        ...
        "bl_img_noresize": {
            "w_route": "./Experiments_img/efficientNEt_noDA1_2/epoch_40.pt",
            "dtype": "img",
            "size": "original",
            "resize_type": None,
            "version": "b0",
        ...
        "bl_irt_noresize": {
            "w_route": "./Experiments_irt/efficientNEt_noDA1_2/epoch_40.pt",
            "dtype": "irt",
            "size": "original",
            "resize_type": None,
            "version": "b0",
        ...
        "4-HyRS": {
            "w_route": "./Experiments_hybrid/efficientNEt_noDA1/epoch_40.pt",
            "dtype": "hybrid",
            "size": "b0",
            "resize_type": None,
            "version": "b0",
        ...
        "4-HynoRS-0Pad": {
            "w_route": "./Experiments_hybrid/efficientNEt_zeroPad/epoch_40.pt",
            "dtype": "hybrid",
            "size": "original",
            "resize_type": "pad",
            "version": "b0",
        ...
        "4-HynoRS-interpol": {
            "w_route": "./Experiments_hybrid/efficientNEt_interpol_noresize/epoch_40.pt",
            "dtype": "hybrid",
            "size": "original",
        ...
        "fv-HynoRS": {
            "w_route": "./Experiments_hybrid/efficientNEt_featvect_noRSize/epoch_40.pt",
            "dtype": "hybrid2",
            "size": "original",
        ...
        "fv-HyRS": {
            "w_route": "./Experiments_hybrid/efficientNEt_featvect_RSize2/epoch_40.pt",
            "dtype": "hybrid2",
            "size": "b0",
            "resize_type": None,
            "version": "b0",
        ...
        "fv-HynoRS-interpol": {
            "w_route": "./Experiments_hybrid/efficientNEt_featvect_noResize_interpol/epoch_40.pt",
            "dtype": "hybrid2",
            "size": "original",
            "resize_type": "interpol",
            "version": "b0",
        },
        "fv-HynoRS-interpol-b3": {
            "w_route": "./Experiments_hybrid/efficientNEt_featvect_noResize_interpol_b3/epoch_40.pt",
            "dtype": "hybrid2",
            "size": "original",
            "resize_type": "interpol",
            "version": "b3",
    ...
    ## Read data infor
    if args.dtype == "irt":
        df_images = pd.read_csv("test_thermal_mat.csv", index_col=0).sort_values(
            by=["path"]
        )
    elif args.dtype == "img":
        df_images = pd.read_csv("test_thermal_img.csv", index_col=0).sort_values(
            by=["path"]
        )
    elif args.dtype in ["hybrid", "hybrid2", "hybrid3"]:
        df_images = pd.read_csv("test_thermal_img.csv", index_col=0).sort_values(
        ...
        df_irt = pd.read_csv("test_thermal_mat.csv", index_col=0).sort_values(
            by=["path"]
        )
    ...
    if args.dtype in ["hybrid", "hybrid2", "hybrid3"]:
        ...
        images_paths = np.concatenate(
            (
                np.reshape(np.array(df_images["path"].values), (690, 1)),
                np.reshape(np.array(df_irt["path"].values), (690, 1)),
            ),
            1,
        )
    ...
    def testNet(args, model, x_test, y_test, dtype, gpuID):
        ...
        test_transforms = transforms.Compose(
            [transforms.ToTensor(), transforms.Normalize([0.53], [0.32])]
        )
        kwargs = (
            {
                "num_workers": 1,
        ...
        if dtype in ["irt", "img"]:
            ...
            print(f"Test {args.dtype} data loading ...")
            if args.size == "b0":
                test_dataset = IRTHybrid(
                    x_test, y_test, ROOT_PATH, transform=None
                )  # transforms.Compose([transforms.ToTensor()]), train_transforms
            else:
                test_dataset = IRTHybrid2(
                    x_test,
                    y_test,
                    ROOT_PATH,
        ...
        test_loader = DataLoader(
            test_dataset, batch_size=args.batchSize, shuffle=True, **kwargs
        )
        ...
        with torch.no_grad():
            ...
            if dtype in ["irt", "img"]:
                ...
                for batch_idx, (data, irt, target) in tqdm.tqdm(
                    enumerate(test_loader), total=len(test_loader)
                ):
                    if args.cuda:
                        data, irt, target = (
            ...
            )  # with accuracy {float(accuracy)* 100:.2f}

        # METRICS
        aca, recall, f1_score, support = precision_recall_fscore_support(
    ...
    def demo(args, model, x_test, y_test, dtype, gpuID):
        ...
        # Apply log_softmax activation to the predictions
        # and pick the index of highest probability.

        print(f"Prediction for {args.img} is: S{predictions.item()}")
        print(f"True label is: S{target.item()}")

    model = model = EfficientNet(
        args.dtype, version=args.version, num_classes=4, resize_type=args.resize_type
    )
    state_dict = torch.load(experiment[args.experiment]["w_route"])
    model.load_state_dict(state_dict["state_dict"], strict=False)

    if args.mode == "test":
        loss_test, accuracy, aca, f1_score, confusion_mx = testNet(
            args, model, x_test, y_test, dtype=args.dtype, gpuID=args.gpuID
        )
        print(
            f"Accuracy score for {args.experiment}\
                is {accuracy*100:.2f}% and average loss is {loss_test:.2f}"
        )
        ...
        results = pd.DataFrame(
            {
                ...
                "ACA": [aca],
                "f1_score": [f1_score],
                "confusion_mat": [confusion_mx.reshape(16)],
            }
        )
